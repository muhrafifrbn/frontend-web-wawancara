...
const ModalProfil = ({ modal }) => {
  return <div>{/* Modal profil content */}</div>;
};
    ...
    <>
      ...
      <div>
              ...
              <div className="flex items-center">
                <div className="flex items-center ms-3">
                  <div>
                    ...
                    >
                      <FaUser className="w-8 h-8 pt-2 text-gray-400 bg-white rounded-full" />
                    </button>
                  </div>
                  <div
                    className={`${
                      showProfile ? "absolute block" : "hidden"
                    } w-[240px] z-50 my-4 text-base list-none bg-white divide-y divide-gray-800 rounded shadow right-4 top-12 border-[0.6px] border-gray-900`}
                  >
        ...
        <aside
          className={`${
            showSide ? "w-64" : "w-0"
          } fixed top-0 left-0 z-40 h-screen transition-all duration-300 bg-gray-800 overflow-hidden border-r border-maroon`}
        >
            ...
            <ul className="pb-4 space-y-2 font-medium ">
              {data.map((e, i) => (
                <li key={i} className="cursor-pointer">
                  <Link to={e.to}>
                    <div
                      className={`flex hover:shadow-lg hover:bg-white items-center gap-2 p-2 text-white rounded-lg group ${
                        e.name === title ? "shadow-lg py-2.5" : ""
                      }`}
                    >
                      <span
                        className={`${
                        ...
                        } flex items-center shadow-md justify-center p-3 transition group-hover:bg-maroon group-hover:text-white duration-75 rounded-lg`}
                      >
                        {e.ic}
                      </span>
                      <span
        ...
        <div
          className={`p-4 mt-[69px] bg-neutral-100/20 transition-all duration-300 ${
            showSide ? "lg:ml-64" : "ml-0"
          }`}
        >
          <div className="border-gray-200 min-h-[620px]">{children}</div>
        </div>
      </div>
    </>
  );
