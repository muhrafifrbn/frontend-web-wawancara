...
const Ortu = () => {
  ...
  const renderParentRow = (item, index) => (
    <tr className="bg-white border-b" key={item.id || index}>
      ...
      <td
        className="px-6 py-4 text-gray-900 truncate"
        title={
      ...
      >
        {truncateText(
          item.additional_info || "Tidak Ada Informasi Tambahan",
          50
        )}
      </td>
      <td className="px-6 py-4 text-gray-900">
  ...
  return (
    <Dashboard title="Orang Tua">
      <div className="flex flex-col justify-between w-full min-h-[700px] xl:min-h-[calc(100vh-130px)]">
        ...
        {showModal && (
          <DetailOrtu id={selectedId} onClose={() => setShowModal(false)} />
        )}
        {showEditModal && (
          <EditOrtuModal
            id={selectedId}
